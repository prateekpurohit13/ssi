  ...
  useEffect(() => {
    ...
    const shuffleText = (originalText: string, currentRevealed: Set<number>): string => {
      if (useOriginalCharsOnly) {
        ...
        const nonSpaceChars = positions.filter((point) => !point.isSpace && !point.isRevealed).map((point) => point.char)

        for (let index = nonSpaceChars.length - 1; index > 0; index -= 1) {
          const shuffleIndex = Math.floor(Math.random() * (index + 1))
          ;[nonSpaceChars[index], nonSpaceChars[shuffleIndex]] = [nonSpaceChars[shuffleIndex], nonSpaceChars[index]]
        }

        let charIndex = 0
        return positions
          .map((point) => {
            if (point.isSpace) return ' '
            if (point.isRevealed) return originalText[point.index]
